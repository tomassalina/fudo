...
Ver `backend/db/structure.sql` (fuente de verdad ejecutable del esquema) y `docs/database-schema.drawio` (diagrama visual). 14 tablas + 7 enums. La pieza central: `consumers` es una identidad global, `visit_summaries`/`visits`/`loyalty_rules` resuelven el vínculo entre esa identidad global y la plataforma multi-tenant existente de Fudo (un tenant = un merchant).
